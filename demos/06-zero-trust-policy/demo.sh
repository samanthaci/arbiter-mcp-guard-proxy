...
if ! kill -0 "$ARBITER_PID" 2>/dev/null; then
  echo -e "${RED}Arbiter exited on startup — is port 8080 or 3000 already in use?${NC}"
  exit 1
fi
if ! curl -sf "$PROXY/health" 2>/dev/null | grep -q '"'"'"status":"healthy"'"'"'; then
  echo -e "${RED}Health check failed — another service may be answering on $PROXY${NC}"
  exit 1
fi
...
echo "  is denied with POLICY_DENIED. A policy evaluation trace is"
echo "  logged server-side for operator diagnostics."
echo ""
